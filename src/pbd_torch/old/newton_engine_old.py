...
class NonSmoothNewtonEngine:
    def __init__(self,
        ...
        self._J_j = torch.zeros((B, 5 * D, 6), device=device)  # [B, 5 * C, 6]
    ...
    def compute_jacobian(
        ...
        D = lambda_j.shape[1] // 5  # Maximum number of joints per body
        total_dim = 6 + 4 * C + 5 * D
        ...
        J_F[:, :6, 6 + 4 * C : 6 + 4 * C + 5 * D] = dres_d_dlambda_j # ∂res_d/∂lambda_j
        ...
        J_F[:, 6 + 4 * C : 6 + 4 * C + 5 * D, :6] = dres_j_dbody_vel # ∂res_j/∂body_vel
        J_F[:, 6 + 4 * C : 6 + 4 * C + 5 * D, 6 + 4 * C : 6 + 4 * C + 5 * D] = dres_j_dlambda_j # ∂res_j/∂lambda_j
        return J_F
    ...
    def perform_newton_step_line_search(
        ...
        D = lambda_j.shape[1] // 5  # Maximum number of joints
        ...
        delta_lambda_j = delta_x[:, 6 + 4 * C : 6 + 4 * C + 5 * D]  # [B, 5 * D]
        ...
        for _ in range(max_linesearch_iters):
            ...
            lambda_j_trial = lambda_j + alpha * delta_lambda_j
    ...
    def simulate(
        ...
        lambda_j = torch.full((B, 5 * D, 1), 0.01, device=self.device) # [B, 5 * D, 1]
