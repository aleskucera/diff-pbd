...
def numerical_qd(
    body_q: torch.Tensor, body_q_prev: torch.Tensor, dt: float
) -> torch.Tensor:
...
def get_ground_contact_deltas(
    ...
    for i in range(contact_count):
        ...
        r_b_world = contact_point_ground[i]
        ...
        dbody_q, _, d_lambda = positional_delta(
            body_q_a=body_q[b],
            body_q_b=body_q_b,
            r_a=r,
            r_b=r_b_world,
...
def get_restitution_deltas(
    ...
    for c in range(contact_count):
        ...
        r_a_world = body_to_world(r, body_q[b])
        r_b_world = contact_point_ground[c]

        if r_a_world[2] - r_b_world[2] >= 0:
            continue

        dbody_qd = ground_restitution_delta(
            body_q=body_q[b],
            body_qd=body_qd[b],
...
def get_joint_deltas(
    ...
    for j in range(num_joints):
        ...
        dbody_q_p, dbody_q_c = joint_deltas(
            body_q_p=body_q_p,
...
def forces_from_joint_actions(
    ...
    num_joints = joint_parent.shape[0]
    body_f = torch.zeros(
        (body_q.shape[0], 6), dtype=torch.float32, device=body_q.device
    )
    ...
    for j in range(num_joints):
        ...
        axis_w = rotate_vector(axis, X_wj_p[3:])

        joint_torque = axis_w * t

        body_f[parent, :3] = body_f[parent, :3] - rotate_vector_inverse(
            joint_torque, body_q_p[3:]
        )
        body_f[child, :3] = body_f[child, :3] + rotate_vector_inverse(
            joint_torque, body_q_c[3:]
        )
...
class XPBDIntegrator:
    ...
    def simulate(
        ...
        for i in range(model.body_count):
            body_q[i], body_qd[i] = integrate_body(
                body_q[i],
                body_qd[i],
                body_f[i],
            ...
            debug.undent()
        debug.print(f"Integration Time: {time.time() - int_time:.6f} s")
        # ======================================== END: INTEGRATION ========================================
        ...
        for i in range(self.iterations):
            ...
            if model.contact_count > 0:
                ...
                for b in range(model.body_count):
                    ...
                    if num_body_corrections == 0:
                        continue
                    delta_q = torch.sum(body_deltas, dim=0) / num_body_corrections
                ...
                debug.print(
                    f"Contact Correction Time: {time.time() - contact_corr_time:.6f} s"
                )
            debug.undent()
            ...
            for b in range(model.body_count):
                ...
                if n_corr[b] > 0:
                    ...
                    body_q[b, 3:] = normalize_quat(
                        body_q[b, 3:] + joint_deltas[b][3:] / n_corr[b]
                    )
            debug.undent()
        ...
        if model.contact_count > 0:
            ...
            for b in range(model.body_count):
                ...
                num_body_corrections = non_zero_mask.sum().item()
                if num_body_corrections != 0:
                    delta_qd = torch.sum(body_deltas, dim=0) / num_body_corrections
