...
class AnimationController:
    ...
    def __init__(
        ...
        self.time = time if time is not None else torch.arange(len(frames_sequence))
    ...
    def plot_frame(self, frame: BodyFrame):
        ...
        # Define axes
        x_axis = rotate_vector(torch.tensor([1.0, 0.0, 0.0]), orientation) * frame.scale
        y_axis = rotate_vector(torch.tensor([0.0, 1.0, 0.0]), orientation) * frame.scale
        z_axis = rotate_vector(torch.tensor([0.0, 0.0, 1.0]), orientation) * frame.scale
    ...
    def on_key_press(self, event):
        if event.key == "n":  # Next frame
            self.current_frame = (self.current_frame + 1) % len(self.frames_sequence)
            self.update_plot()

        elif event.key == "N":  # Jump forward by 10 frames
            self.current_frame = (self.current_frame + 10) % len(self.frames_sequence)
            self.update_plot()
