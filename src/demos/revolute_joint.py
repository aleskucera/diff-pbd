...
from pbd_torch.transform import transform
...
def show_joint():
    ...
    wheel = model.add_cylinder(m=1.0,
                               radius=2.0,
                               height=0.4,
                               name='left_wheel',
    ...
    base_points = transform(model.body_collision_points[base],
                            model.body_q[base])
    wheel_points = transform(model.body_collision_points[wheel],
                             model.body_q[wheel])
    ...
    plot_points(ax, base_points, "Base Body")
    plot_points(ax, wheel_points, "Wheel Body")

    # Plot the body frames (base is larger than wheel)
    plot_frame(ax, model.body_q[base], 'r', scale=1.0, label='Base Frame')
    plot_frame(ax, model.body_q[wheel], 'b', scale=0.5, label='Wheel Frame')
