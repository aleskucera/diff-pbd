...
INITIAL_POSITION = Vector3(torch.tensor([-0.5, -0.0, 4.5]))
...
experiment_dir = os.path.join(data_dir, "sphere_bounce_grad2")
...
def compute_numerical_gradients(
    ...
    for velocity in velocities:
        ...
        loss_original = compute_loss(states[-1].body_q[sphere_idx], target_transform)
        ...
        loss_perturbed = compute_loss(states_perturbed[-1].body_q[sphere_idx], target_transform)
...
def compute_loss(body_q: torch.Tensor, target_body_q: torch.Tensor) -> torch.Tensor:
    ...
    return torch.norm(body_q[2] - target_body_q[2], p=2)
