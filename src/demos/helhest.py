...
from pbd_torch.old.xpbd_integrator2 import XPBDIntegrator as XPBDIntegrator2
...
def main():
    dt = 0.02
    n_steps = 200
    output_file = os.path.join("simulation", "helhest.json")
    ...
    model = Model(terrain=terrain, device=torch.device("cpu"))

    engine = XPBDEngine(iterations=2)
    # integrator = XPBDIntegrator2(iterations=3)
