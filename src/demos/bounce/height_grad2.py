...
ALL_INTEGRATOR_TYPES = ['adaptive_icpg']
...
def analytical_height(x0: torch.Tensor, v0: torch.Tensor, t: float, restitution: float):
    ...
    if t < t_impact:
        ...
        return x0[1] + v0[1] * t_impact - restitution * v0[1] * (t - t_impact)
...
def main():
    ...
    py_range_sweep = np.linspace(0.5, 1.5, 31)
    ...
    integrator_config = {
        ...
        'adaptive_icpg': {'label': 'Hybrid Approach', 'color': '#2ca02c', 'linewidth': 2.0}
    }
    ...
    for integrator, integrator_cfg in integrator_config.items():
        ...
        y_labels_for_scientific[grad_subplot_title_key] = r'$\displaystyle\pdv{l}{x_z^0}$ (-)'
        plot_titles_for_scientific[grad_subplot_title_key] = fr'Gradient for {integrator_cfg["label"]}'

    num_total_subplots = len(ALL_INTEGRATOR_TYPES) + 2  # Loss + Integrator Gradients + Analytical Gradient
