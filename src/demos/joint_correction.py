...
def joint_correction_demo():
    ...
    frames_sequence.append({
        'parent_body':
        BodyFrame(body_q_p, scale=1.0, color='purple', label='Parent Body'),
        'parent_joint':
        BodyFrame(X_wj_p, scale=1.0, color='orange', label='Parent Joint'),
        'child_body':
        BodyFrame(body_q_c, scale=0.5, color='green', label='Child Body'),
        'child_joint':
        BodyFrame(X_wj_c, scale=0.5, color='blue', label='Child Joint')
    })

    # Perform iterations and store frames
    for _ in range(5):
        dbody_q_p, dbody_q_c = joint_delta(body_q_p, body_q_c, X_p, X_c,
                                           torch.tensor([1.0, 0.0, 0.0]),
                                           torch.tensor([1.0]),
                                           torch.tensor([1.0]), torch.eye(3),
                                           torch.eye(3))
        body_q_p += dbody_q_p
        body_q_c += dbody_q_c
        ...
        frames_sequence.append({
            'parent_body':
            BodyFrame(body_q_p, scale=1.0, color='purple',
                      label='Parent Body'),
            'parent_joint':
            BodyFrame(X_wj_p, scale=1.0, color='orange', label='Parent Joint'),
            'child_body':
    ...
    controller = AnimationController(frames_sequence,
                                     x_lims=(-1, 6),
                                     y_lims=(-3, 3),
                                     z_lims=(-3, 3))
    controller.start()
